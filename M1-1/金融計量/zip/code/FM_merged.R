...
# =============================================Stock Portfolio
# Checking
stock_portfolio[1:3,1:45]
dim(stock_portfolio)

# Transfer to long format
long_data = pivot_longer(stock_portfolio,
                         cols=2:ncol(stock_portfolio),
                         names_to = 'portf',
                         values_to = 'ExRet')
...
# NWSE
Mkt_RP_stock = FM_reg2$indcoef['rm',]
SMB_RP_stock = FM_reg2$indcoef['SMB',]
HML_RP_stock = FM_reg2$indcoef['HML',]

coeftest(lm(Mkt_RP_stock ~ 1), vcov=NeweyWest,lag=3)
coeftest(lm(SMB_RP_stock ~ 1), vcov=NeweyWest,lag=3)
coeftest(lm(HML_RP_stock ~ 1), vcov=NeweyWest,lag=3)
...
# NWSE_etf
Mkt_RP_etf = FM_reg2$indcoef['rm',]
SMB_RP_etf = FM_reg2$indcoef['SMB',]
HML_RP_etf = FM_reg2$indcoef['HML',]

coeftest(lm(Mkt_RP_etf ~ 1), vcov=NeweyWest,lag=3)
coeftest(lm(SMB_RP_etf ~ 1), vcov=NeweyWest,lag=3)
coeftest(lm(HML_RP_etf ~ 1), vcov=NeweyWest,lag=3)
# =================================================\lambda_D
a = Mkt_RP_stock - Mkt_RP_etf
coeftest(lm(a ~ 1), vcov=NeweyWest,lag=3)

b = HML_RP_stock - HML_RP_etf
coeftest(lm(b ~ 1), vcov=NeweyWest,lag=3)

c = SMB_RP_stock - SMB_RP_etf
coeftest(lm(c ~ 1), vcov=NeweyWest,lag=3)
